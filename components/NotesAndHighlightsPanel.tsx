...
const NotesAndHighlightsPanel: React.FC<NotesAndHighlightsPanelProps> = ({ bookId }) => {
  ...
  const handleDeleteNote = (noteId: string) => {
    if (window.confirm(label('confirmDeleteNote', 'Are you sure you want to delete this note?'))) {
      deleteNote(noteId);
    }
  };

  const handleDeleteHighlight = (highlightId: string) => {
    if (window.confirm(label('confirmDeleteHighlight', 'Are you sure you want to delete this highlight?'))) {
    ...
    return (
      <div className="bg-gray-50 rounded-lg p-6 text-center">
        <p className="text-gray-600 mb-4">
          {label('loginToAddNotes', 'Log in to save personal notes and highlights.')}
        </p>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-xl shadow-lg border border-gray-100">
      <div className="border-b border-gray-200">
        <div className="flex">
          ...
          >
            {label('notes', 'Notes')} ({notes.length})
          </button>
          <button
          ...
          >
            {label('highlights', 'Highlights')} ({highlights.length})
          </button>
        </div>
      </div>
            ...
            {notes.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500 mb-4">
                  {label('noNotesYet', 'No personal notes yet.')}
                </p>
                <p className="text-sm text-gray-400">
                  {label('addNotesHint', 'Add notes while reading to capture your thoughts and insights.')}
                </p>
              </div>
            ) : (
                ...
                {notes.map((note) => (
                  <div key={note.id} className="bg-blue-50 rounded-lg p-4 border-l-4 border-blue-500">
                    ...
                    <div className="text-xs text-gray-500">
                      {label('created', 'Created')}: {note.createdAt.toLocaleDateString()}
                      {note.updatedAt.getTime() !== note.createdAt.getTime() &&
                        ` • ${label('updated', 'Updated')}: ${note.updatedAt.toLocaleDateString()}`
                      }
                    </div>
                  </div>
                ))}
          ...
          <div>
            {highlights.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500 mb-4">
                  {label('noHighlightsYet', 'No highlights yet.')}
                </p>
                <p className="text-sm text-gray-400">
                  {label('highlightTextHint', 'Highlight important text while reading to save key passages.')}
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                {highlights.map((highlight) => (
                  <div key={highlight.id} className="bg-yellow-50 rounded-lg p-4 border-l-4 border-yellow-500">
                    ...
                    <div className="text-xs text-gray-500">
                      {label('created', 'Created')}: {highlight.createdAt.toLocaleDateString()}
                      {highlight.updatedAt.getTime() !== highlight.createdAt.getTime() &&
                        ` • ${label('updated', 'Updated')}: ${highlight.updatedAt.toLocaleDateString()}`
                      }
                    </div>
                  </div>
                ))}
