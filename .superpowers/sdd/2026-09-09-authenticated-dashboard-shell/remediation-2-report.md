...
- Verification: `node --import tsx --test tests/dashboard*.test.ts` passed 34/34; full `node --import tsx --test tests/*.test.ts tests/*.test.tsx` passed 131/131; `npm run build` passed sitemap, Vite, prerender, and SEO tests; `git diff --check` passed.
